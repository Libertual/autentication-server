...
app.put('/api/pass', ensureAuthenticated, function(req, res) {
  User.findById(req.user, '+password', function(err, user) {
    ...
    user.comparePassword(req.body.oldPassword, function(err, isMatch) {
      if (!isMatch) {
        return res.status(401).send({ message: 'Tu antigua contraseña no es correcta' });
      }
      else{
      user.password = req.body.newPassword || user.password;
      user.save(function(err) {
        console.log('Contraseña cambiada a :' + user.password);
